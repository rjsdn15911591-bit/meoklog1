...
│   │   │   └── Toast.tsx              # 전역 Toast 알림 컴포넌트 (fixed bottom-24)
│   │   ├── meal/
│   │   │   ├── MealCard.tsx           # 식사 카드 (사진+칼로리, 빈 이미지 폴백 UI)
│   │   │   ├── MealUploadForm.tsx     # 사진 업로드 + AI 결과 수정 폼
│   │   │   ├── FoodSearchModal.tsx    # 음식 검색 + 즐겨찾기 (백엔드 DB 동기화)
│   │   │   ├── AddCustomFoodModal.tsx # 음식 직접 추가 (3단계 폼)
│   │   │   ├── QuickLogModal.tsx      # 텍스트 퀵로그 모달 (사진 없는 식사 입력)
│   │   │   └── NutritionDetail.tsx    # 탄단지 상세 (targets 프롭으로 개인 목표 대비)
│   │   ├── group/
│   │   │   ├── GroupFeed.tsx          # 그룹 피드
│   │   │   ├── GroupJoinModal.tsx     # 그룹 생성/참가 모달
│   │   │   └── GroupSettingsModal.tsx # 그룹 설정 (오너 전용)
│   │   ├── analysis/
│   │   │   ├── DailyAnalysis.tsx      # 하루 분석 카드 (개인 목표 targets 전달)
│   │   │   ├── NutritionChart.tsx     # 영양소 도넛 차트 (단백질 109g 기준)
│   │   │   ├── WeeklyTrendChart.tsx   # 주간 칼로리 바 차트
│   │   │   ├── MonthlyStats.tsx       # 월간 통계 (달력 히트맵 + Top5 음식)
│   │   │   ├── WeightTracker.tsx      # 체중 기록 SVG 꺾은선 그래프 + CRUD
...
│   │   └── useToast.ts                # Toast 알림 훅 (visible/message/show)
...
│   │   │   ├── food_item.py
│   │   │   ├── weight_record.py       # WeightRecord (체중 기록)
│   │   │   └── favorite.py            # UserFavoriteFood (즐겨찾기 DB 동기화)
