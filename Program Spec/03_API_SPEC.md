...
Response 200:
{
  ...
  "data": {
    ...
    "total_fat": 42,
    "target_calories": 2200,
    "target_carbs": 275,        // v1.6 추가 — 개인 목표 탄수화물(g)
    "target_protein": 138,      // v1.6 추가 — 개인 목표 단백질(g)
    "target_fat": 49,           // v1.6 추가 — 개인 목표 지방(g)
...
// target_carbs/protein/fat 계산:
// users.target_* 가 있으면 그 값 사용
// 없으면 target_calories 기준 자동 계산:
//   target_carbs   = round(target * 0.55 / 4)
//   target_protein = round(target * 0.25 / 4)
//   target_fat     = round(target * 0.20 / 9)
```

### GET /users/me/weight 🔐 — v1.6
체중 기록 조회

```
Query Params:
  days: int (기본값: 90, 최대: 365) — 최근 N일 기록

Response 200:
{
  "success": true,
  "data": [
    { "id": "uuid", "weight": 70.5, "recorded_at": "2026-06-22", "note": null },
    ...
  ]
}
```

### POST /users/me/weight 🔐 — v1.6
체중 기록 추가 (같은 날짜면 덮어쓰기)

```
Request Body:
{ "weight": 70.5, "date": "2026-06-22", "note": "아침 공복" }  // date 생략 시 오늘

Response 200:
{
  "success": true,
  "data": { "id": "uuid", "weight": 70.5, "recorded_at": "2026-06-22", "note": "아침 공복" }
}
```

### DELETE /users/me/weight/{record_id} 🔐 — v1.6
체중 기록 삭제
...
Response 200:
{
  ...
  "data": [
    {
      "id": "uuid", "food_name": "흰쌀밥", "brand_name": null,
      "serving_size": 200.0, "serving_unit": "g",
      "calories": 260.0, "carbs": 56.2, "protein": 5.0, "fat": 0.6,
      "source": "system", "is_public": true, "use_count": 12
    }
  ]
}
```

### POST /users/me/favorites/{food_id} 🔐 — v1.6
즐겨찾기 추가
...
// 이미 즐겨찾기인 경우 → 동일하게 200 반환 (중복 무시)
// 존재하지 않는 food_id → 404
```

### DELETE /users/me/favorites/{food_id} 🔐 — v1.6
즐겨찾기 제거
...
### GET /users/me/monthly-stats 🔐 — v1.6
월간 통계 (달력 히트맵 데이터 + Top5 음식)
...
Response 200:
{
  ...
  "data": {
    ...
    "top_foods": [
      { "food_name": "흰쌀밥", "count": 18, "total_calories": 4680 },
      ...
    ],
    "summary": {
      "logged_days": 22,
...
### POST /meals/quick 🔐 — v1.6
사진 없는 텍스트 퀵로그 (음식 직접 입력)
...
// 프론트엔드 처리:
//   imageUrl === "" → PenLine 아이콘 + "텍스트 기록" 폴백 UI
//   MealCard, 식사 상세 페이지 모두 동일 처리
```
