...
## 22. 텍스트 퀵로그 (`QuickLogModal`) — v1.6 추가

사진 없이 음식명·칼로리·탄단지를 직접 입력해 식사 기록.

### 진입 경로
카메라 탭 (`/camera`) 하단 "또는" 구분선 아래 **"텍스트로 직접 기록하기"** 버튼.
단, 이 버튼은 `step === 'select'` 상태에서만 표시 (AI 분석 중·결과 화면에서는 숨김).

### 흐름
1. 버튼 클릭 → `QuickLogModal` 전체화면 모달 열림
2. 식사 타입 선택 (아침/점심/저녁/간식)
3. FoodSearchModal로 음식 검색·담기
4. 담긴 음식 목록 확인 + 합계 표시
5. "저장" 클릭 → `POST /meals/quick` 호출
6. 저장 성공 → "식사가 기록됐어요!" Toast + 쿼리 무효화

### 빈 이미지 폴백 UI
퀵로그 저장 시 `image_url = ""`, `thumbnail_url = ""`로 저장.

```tsx
// MealCard.tsx, meal/[mealId]/page.tsx 공통 처리
{meal.thumbnailUrl || meal.imageUrl ? (
  <Image src={meal.thumbnailUrl || meal.imageUrl} ... />
) : (
...
## 23. 체중 기록 트래커 (`WeightTracker`) — v1.6 추가

분석 탭 > 월간 탭 하단에서 체중 추이를 기록·조회.
...
### API
```
GET    /users/me/weight?days=90   → 체중 기록 목록
POST   /users/me/weight           → 기록 추가 (같은 날 덮어쓰기)
DELETE /users/me/weight/{id}      → 기록 삭제
```

---

## 24. 월간 통계 (`MonthlyStats`) — v1.6 추가

분석 탭 > 월간 탭에서 한 달 식단 성과 시각화.
...
│  < 2026년 6월 >                          │
│                                         │
│  요약:  기록 22일 / 평균 1,703kcal / ...  │
│                                         │
│  [달력 히트맵 — 7×6 그리드]              │
│   칸 색상: 달성률에 따라 sage→ochre→coral │
│   회색: 기록 없음                        │
│                                         │
│  이번 달 자주 먹은 음식 Top 5            │
│  1. 흰쌀밥  18회  4,680kcal             │
...
// 위치: fixed bottom-24, z-[300], 좌우 중앙 정렬
// 애니메이션: opacity + translateY transition 300ms
...
## 26. 영양소 개인 목표 대비 (`NutritionDetail`) — v1.6 추가

### targets 프롭
`targets` 프롭이 있으면 DRI(식약처 기준치) 대신 개인 목표 대비 % 계산.
...
<NutritionDetail
  ...
  targets={
    summary.targetCarbs && summary.targetProtein && summary.targetFat
      ...
      : undefined  // 개인 목표 미설정 시 DRI 기준치 사용
  }
/>
```
...
푸터 텍스트:
- `targets` 있음: `* 개인 목표 대비`
- `targets` 없음: `* 1일 영양성분 기준치(2,000kcal) 대비`

### 단백질 기준치 통일
`NutritionDetail.DRI.protein = 109` (구: 55)
`NutritionChart.DAILY_REF.protein = 109`

두 컴포넌트 모두 109g으로 통일. 식약처 2020 개정 기준치(성인 기준) 적용.
