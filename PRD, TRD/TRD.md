...
│  │  ┌─────────────────────────────────────────────────┐    │   │
│  │  │          AI 분석 서비스 (싱글톤)                  │    │   │
│  │  │  MobileNetV2 → food_labels.json → food_items DB │    │   │
│  │  └─────────────────────────────────────────────────┘    │   │
│  └──────────────────────────────────────────────────────────┘   │
└──────────┬──────────────────────┬───────────────────────────────┘
           │                      │
...
[FastAPI: POST /meals]
    ...
    │
    ├─2─▶ [AI Service: MobileNetV2] 이미지 분석
    │         ↓
    │     Top-3 음식 예측 + confidence 반환
    │
    ├─3─▶ [food_items DB] 영양 정보 조회
    │         ↓
    │     칼로리/탄단지 계산
    │
    ├─4─▶ [Supabase PostgreSQL] meal_records + detected_foods 저장
    │
    └─5─▶ [Supabase Realtime] 그룹 채널에 INSERT 이벤트 발행
              │
              ▼
[그룹원 클라이언트들]
Supabase Realtime 구독 → TanStack Query 캐시 무효화 → 피드 자동 갱신
```
...
| Python | 3.11.x | TensorFlow/Pillow 호환 최고, AI 팀 동일 언어 | Node.js (AI 라이브러리 부재) |
...
| 기술 | 버전 | 선택 근거 | 대안 고려 |
|------|------|----------|----------|
| TensorFlow | 2.15.x | MobileNetV2 공식 지원, 기존 수업 환경과 동일 | PyTorch (MobileNetV2 동일 지원, 마이그레이션 비용) |
| MobileNetV2 | - | 경량화 CNN, 웹 서버 추론 속도 우수, Transfer Learning 검증 완료 | ResNet50 (무거움), EfficientNet (복잡도) |
...
```
모델:        MobileNetV2 (Transfer Learning)
입력:        (1, 224, 224, 3) — RGB 이미지, 정규화됨
출력:        (1, N) — N개 음식 클래스 확률 분포
분류 클래스: Food-101 (101개) + 한국 음식 추가 목표
저장 형식:   .h5 (Keras), 경로: ai_model/weights/mobilenetv2_food.h5
```

### 5.2 이미지 전처리 파이프라인

```python
# app/ai_model/preprocess.py
...
def preprocess_for_mobilenetv2(image_bytes: bytes) -> np.ndarray:
    """
    1. bytes → PIL Image (RGB 강제 변환)
    2. 224×224 리사이즈 (LANCZOS 알고리즘 — 품질 우선)
    3. numpy array 변환
    4. MobileNetV2 정규화: [-1, 1] 범위로 변환
    5. 배치 차원 추가: (224, 224, 3) → (1, 224, 224, 3)
    """
...
```python
def predict_top_k(image_bytes: bytes, top_k: int = 3) -> list[dict]:
    """
    Returns:
        [
            {"food_name": "흰쌀밥", "confidence": 0.94, "label_index": 0},
            {"food_name": "볶음밥",  "confidence": 0.03, "label_index": 7},
            {"food_name": "덮밥",    "confidence": 0.02, "label_index": 12},
        ]
    """
    processed = preprocess_for_mobilenetv2(image_bytes)
    predictions = model.predict(processed, verbose=0)[0]
    top_indices = np.argsort(predictions)[-top_k:][::-1]
    
    return [
        {
            "food_name":   labels[str(idx)],
            "confidence":  float(predictions[idx]),
            "label_index": int(idx)
        }
        for idx in top_indices
        if float(predictions[idx]) > 0.01  # 1% 미만 노이즈 제거
    ]
```
...
```
AI 추론 결과 food_name → food_items 테이블 조회 → 100g 기준 영양 정보 반환

매핑 전략:
  1. food_name_en (영문명) 완전 일치 검색 (Food-101 라벨 직접 매핑)
  2. food_name (한글명) 검색
  3. 두 방법 모두 실패 시: "알 수 없는 음식" + 사용자에게 수동 입력 요청

기본 제공량(serving_size):
  DB에 저장된 100g 기준 영양값 × (실제 제공량 / 100)
  기본 제공량은 food_items.serving_size 컬럼 참조
```
...
class FoodAIService:
    """
    앱 시작 시 1회 모델 로드, 이후 메모리 유지.
    모든 요청에서 동일 인스턴스 재사용 (Thread-safe read-only).
    """
    _instance = None
    _model = None
    _labels = None
...
async def lifespan(app: FastAPI):
    food_ai_service.load_model()  # 앱 시작 시 1회만 실행
...
```
실패 케이스:
  1. 모델 로드 실패    → 500 에러, 수동 입력 UI 표시
  2. 이미지 전처리 실패 → 400 에러 "지원하지 않는 이미지 형식"
  3. confidence 모두 10% 미만 → "음식을 인식하지 못했어요" + 수동 입력 폼
  4. food_items DB 매핑 실패 → "알 수 없는 음식" + 사용자 직접 수정 유도
